...
var log = (0, _debug2['default'])('revenge:@pure');
...
function shallowEqual(objA, objB, section, component) {
  ...
  for (key in objA) {
    if (objA.hasOwnProperty(key) && (!objB.hasOwnProperty(key) || objA[key] !== objB[key])) {
      if (process.env.NODE_ENV !== 'production') {
        log('component ' + displayName + ' with rootNodeID ' + rootNodeID + ' will re-render since ' + section + ' key ' + key + ' is changed from ' + (0, _stringify2['default'])(objA[key]) + ' to ' + (0, _stringify2['default'])(objB[key]));
      }
      return false;
    }
  }
  ...
  for (key in objB) {
    if (objB.hasOwnProperty(key) && !objA.hasOwnProperty(key)) {
      if (process.env.NODE_ENV !== 'production') {
        log('component ' + displayName + ' with rootNodeID ' + rootNodeID + ' will re-render since ' + section + ' key ' + key + ' with value ' + (0, _stringify2['default'])(objB[key]) + ' is new');
      }
      return false;
    }
  }
